...
| `siddur.html` | Live siddur. Loads the table of contents and every text at runtime from the **Sefaria API** (Siddur Ashkenaz / Sefard / Edot HaMizrach), English alongside the Hebrew where a translation exists (segment level — not word-by-word interlinear; Sefaria does not publish word alignment), graceful Hebrew-only fallback where it doesn't, per-section license attribution, local caching. Checks the Supabase shared cache first, falls back to Sefaria directly. |
...
| `supabase/migrations/20260726001548_luach_app_initial_schema.sql` | The applied database schema (already live). The filename's timestamp matches the version recorded in the project's migration history, so tooling treats it as applied rather than pending. |
