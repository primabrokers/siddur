-- ============================================================================
-- 001_initial_schema — Yeshiva Donor CRM schema v2, adapted for Supabase
-- ============================================================================
-- Executable form of docs/donor-crm/spec/02-DATA-MODEL.md; derived from
-- docs/donor-crm/schema-v2.sql. Target: Postgres 17 (eu-west-2).
--
-- Supabase adaptations vs schema-v2.sql:
--   * pg_trgm is installed into the `extensions` schema (Supabase convention),
--     so trigram operator classes are qualified as extensions.gin_trgm_ops.
--   * pg_cron installs into pg_catalog (its jobs live in the `cron` schema);
--     the nightly schedule itself is created in 003.
--   * team_members.id keeps its FK to auth.users (Supabase Auth is the identity
--     source; RLS in 002 joins auth.uid() -> team_members).
-- Every table, index and check from schema-v2.sql is preserved verbatim.
-- ============================================================================
...
create index contacts_name_trgm on contacts using gin ((first_name || ' ' || last_name) extensions.gin_trgm_ops);
create index contacts_org_trgm  on contacts using gin (coalesce(organization,'') extensions.gin_trgm_ops);
