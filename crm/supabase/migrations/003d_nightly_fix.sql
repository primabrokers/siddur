-- ============================================================================
-- 003d_nightly_fix — corrects run_nightly() as shipped in 003c.
-- ============================================================================
-- Two fixes, both found by executing the function against real data:
--
--  1. `pledge_chase` aggregated the relationship owner with max(uuid), which
--     has no such aggregate in Postgres. The owner is constant per contact, so
--     it moves into the GROUP BY instead.
--
--  2. `engagement_recompute` wrote every contact row every night, which meant
--     an audit_log row per contact per run (11 §4 keeps audit for the life of
--     the system). The update is now restricted to contacts whose score or
--     tier actually changed, so a second run in the same day is a genuine
--     no-op — which is also what makes the run idempotent end to end.
--
-- Everything else is byte-identical to 003c.
-- ============================================================================
