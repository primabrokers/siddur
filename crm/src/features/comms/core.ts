...
export function folderAfter(
  action: FolderAction,
  // The folder it is leaving. Not consulted today — restore always returns a
  // message to its *home* folder rather than to wherever it happened to be —
  // but kept in the signature because every caller has it and a future
  // "restore to previous" would need nothing else.
  _current: EmailFolder,
  direction: 'in' | 'out',
): EmailFolder {
...
export function textFromHtml(html: string | null | undefined): string {
  return (html ?? '')
    ...
    .replace(/<br\s*\/?>/gi, '\n')
    // A paragraph break is a blank line; a <br> is a single one.
    .replace(/<\/p>/gi, '\n\n')
    .replace(/<[^>]+>/g, ' ')
    .replace(/&nbsp;/gi, ' ')
    .replace(/&amp;/gi, '&')
    ...
    .replace(/&#39;/gi, "'")
    .replace(/[ \t]+/g, ' ')
    // A stripped `<p>` leaves a space either side of the break we kept, so the
    // padding goes before the blank lines are counted.
    .replace(/[ \t]*\n[ \t]*/g, '\n')
    .replace(/\n{3,}/g, '\n\n')
    .trim()
}
...
function addressList(value: unknown): string[] {
  ...
  // A number or a boolean in an address field is a malformed payload, not an
  // address: `to: 42` must yield no recipients, never the recipient "42".
