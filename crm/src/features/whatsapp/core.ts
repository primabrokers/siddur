...
/**
 * The GET subscription handshake, as a decision.
 *
 * Meta calls the webhook once with `hub.mode=subscribe`, `hub.verify_token`
 * and `hub.challenge`; echoing the challenge back verbatim is what proves we
 * own the URL. Three outcomes and no fourth:
 *
 *   503  no `WA_VERIFY_TOKEN` configured — the honest answer, and the one the
 *        Settings card probes for. An admin mid-setup reads it as "the secret
 *        is missing", not as "Meta rejected you".
 *   403  a token was configured and this is not it. No body detail: the
 *        endpoint must not confirm what the right answer looks like.
 *   200  the challenge, as plain text, byte for byte.
 */
export interface WaHandshake {
  status: number
  body: string
  contentType: 'text/plain'
}

export function handshakeDecision(input: {
  verifyToken: string | null | undefined
  mode: string | null | undefined
  token: string | null | undefined
  challenge: string | null | undefined
}): WaHandshake {
  const configured = (input.verifyToken ?? '').trim()
  if (configured === '') {
    return { status: 503, body: 'wa_unconfigured', contentType: 'text/plain' }
  }
  if (input.mode !== 'subscribe' || input.challenge === null || input.challenge === undefined) {
    return { status: 403, body: 'Forbidden', contentType: 'text/plain' }
  }
  if (!constantTimeEquals(input.token ?? '', configured)) {
    return { status: 403, body: 'Forbidden', contentType: 'text/plain' }
  }
  return { status: 200, body: input.challenge, contentType: 'text/plain' }
}

/** Constant-time string compare for the verify token (not a hex digest). */
