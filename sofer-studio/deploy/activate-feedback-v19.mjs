// Scope-locked v19 cutover. Only the Sofer site block changes.
...
const expected=readFileSync(new URL('./sofer-site-before-v19.caddy',import.meta.url),'utf8').replace(/\r\n/g,'\n').trim();
if(original!==expected)throw new Error('Sofer routing changed since audit; stop for review');
const block=readFileSync(new URL('./sofer-site-v19.caddy',import.meta.url),'utf8').replace(/\r\n/g,'\n').trim();
if(!block.includes('sofer-studio-app-v19:8080')||!block.includes('sofer-studio-demo-v19:8080'))throw new Error('Missing v19 route');
...
console.log(JSON.stringify({validated:true,apply,change:'Sofer v19; preserve active older temporary demos unless explicitly upgraded',otherHostBytesHash:hash(old.slice(0,start)+old.slice(end))}));
if(!apply)process.exit(0);
for(const name of ['prima-sofer-studio-v19','prima-sofer-demo-v19','prima-sofer-studio-v18','prima-sofer-demo-v18']){
...
const backup='/opt/sofer-studio/backups/Caddyfile.before-feedback-v19-'+Date.now();copyFileSync(path,backup);chmodSync(backup,0o600);
