...
const result = db.transaction(() => {
  ...
  return Object.fromEntries(tables.map(({ name }) => {
    ...
    for (const row of db.prepare('SELECT * FROM ' + quote(name) + ' ORDER BY ' + (order.join(',') || 'rowid')).iterate()) {
      if (name === 'schema_migrations' && row.version === 8) continue;
      if (name === 'profiles') {
        if (row.letter_height_units != null) throw new Error('A profile has changed height units during release');
        delete row.letter_height_units;
      }
