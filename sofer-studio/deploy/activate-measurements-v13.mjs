// Scope-locked v13 cutover. Only the Sofer site block changes.
...
const expected=readFileSync(new URL('./sofer-site-before-v13.caddy',import.meta.url),'utf8').trim();
if(original!==expected)throw new Error('Sofer routing changed since audit; stop for review');
const block=readFileSync(new URL('./sofer-site-v13.caddy',import.meta.url),'utf8').trim();
if(!block.includes('sofer-studio-app-v13:8080')||!block.includes('sofer-studio-demo-v13:8080'))throw new Error('Missing v13 route');
...
console.log(JSON.stringify({validated:true,apply,change:'Sofer v13; preserve active older temporary demos unless explicitly upgraded',otherHostBytesHash:hash(old.slice(0,start)+old.slice(end))}));
if(!apply)process.exit(0);
for(const name of ['prima-sofer-studio-v13','prima-sofer-demo-v13','prima-sofer-studio-v12','prima-sofer-demo-v12']){
...
const backup='/opt/sofer-studio/backups/Caddyfile.before-measurements-v13-'+Date.now();copyFileSync(path,backup);chmodSync(backup,0o600);
