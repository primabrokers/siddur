// Scope-locked v15 cutover. Only the Sofer site block changes.
...
const expected=readFileSync(new URL('./sofer-site-before-v15.caddy',import.meta.url),'utf8').trim();
if(original!==expected)throw new Error('Sofer routing changed since audit; stop for review');
const block=readFileSync(new URL('./sofer-site-v15.caddy',import.meta.url),'utf8').trim();
if(!block.includes('sofer-studio-app-v15:8080')||!block.includes('sofer-studio-demo-v15:8080'))throw new Error('Missing v15 route');
...
console.log(JSON.stringify({validated:true,apply,change:'Sofer v15; preserve active older temporary demos unless explicitly upgraded',otherHostBytesHash:hash(old.slice(0,start)+old.slice(end))}));
if(!apply)process.exit(0);
for(const name of ['prima-sofer-studio-v15','prima-sofer-demo-v15','prima-sofer-studio-v14','prima-sofer-demo-v14']){
...
const backup='/opt/sofer-studio/backups/Caddyfile.before-controls-v15-'+Date.now();copyFileSync(path,backup);chmodSync(backup,0o600);
